  ...
  useEffect(() => {
    ...
    const startGather = (fromScatter = true) => {
      ...
      particles.forEach(particle => {
        if (fromScatter) {
          ...
          const noiseAngle = particle.depth * Math.PI * 2 * 10;
          const noiseRadius = particle.seed * spread * 0.45;
          particle.x = particle.targetX + Math.cos(angle) * distance + Math.cos(noiseAngle) * noiseRadius;
          particle.y = particle.targetY + Math.sin(angle) * distance + Math.sin(noiseAngle) * noiseRadius;
        }
    ...
    const sampleText = async () => {
      ...
      particles = selected.map((target, index) => {
        ...
        const noiseRadius = seed * scatter * 0.45;
        const startX = target.x + Math.cos(angle) * distance + Math.cos(noiseAngle) * noiseRadius;
        const startY = target.y + Math.sin(angle) * distance + Math.sin(noiseAngle) * noiseRadius;
